...
import { listDevices } from '@/server/modules/device/device-service';
import {
  ErrorCode,
 ...
 * 查询参数：
 * - appId?: 应用 ID
 * - status?: 设备状态（online/offline/blacklisted）
...
const VALID_STATUSES = ['online', 'offline', 'blacklisted'];
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    const result = await listDevices({ appId, status, limit, offset });
