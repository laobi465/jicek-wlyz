...
- [规划中] 在线充值（彩虹易支付）
- [规划中] 应用管理（创建 / 配置 / AppKey + client_secret）
- [规划中] 卡密管理（7 种类型 / 批量生成 / 模板 / 签名）
- [规划中] 设备管理（绑定 / 心跳 / 黑名单 / 解绑）
- [规划中] 云变量（KV 配置池 / 签名下发）
...
**待核实**：
- ~~服务器系统版本~~ → 已确认 Debian
- 脚本托管 URL（推荐 jsDelivr 加速 GitHub raw：`https://fastly.jsdelivr.net/gh/laobi465/jicek-wlyz@main/deploy/install.sh`，待用户最终确认）
...
> 已确认项见 §1.4 与下方已确认汇总；本表仅列未确认项

| # | 项目 | 状态 |
|---|---|---|
| 1 | 服务器配置（CPU/内存/带宽） | 待核实 |
| 2 | 是否前置 Cloudflare CDN | 待核实 |
| 3 | 是否需要 ICP 备案（国内服务器） | 待核实 |
| 4 | 邮件服务 SMTP 选型 | 待核实 |
| 5 | 短信服务选型 | 待核实 |
