...
| 文件存储 | 对象存储（待核实：七牛/阿里 OSS/Cloudflare R2） |
| 部署 | 自建 VPS + 宝塔面板 + Docker（SSH 一键安装脚本） |
...
**待核实**：
- 服务器系统版本（CentOS / Ubuntu / Debian，影响宝塔安装命令分支）
- 脚本托管 URL（待项目仓库建立后确定）
...
| # | 项目 | 状态 |
|---|---|---|
| 1 | 服务器系统版本（CentOS / Ubuntu / Debian） | 待核实 |
| 2 | GitHub 仓库归属与名称（项目代码仓库新建分支存放 web-project-flow skill） | 待核实 |
| 3 | 彩虹易支付：已有现成商户号？还是脚本预留配置项由管理员后台填？ → 用户已答"管理员配置" | 已确认 |
| 4 | 对象存储选型（七牛 / 阿里 OSS / Cloudflare R2） | 待核实 |
| 5 | 域名已有（用户已确认） | 已确认 |
| 6 | SSH 安装脚本托管 URL | 待核实 |
