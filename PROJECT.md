...
| 1.6.3 | 2026-07-23 | **修复开发者和管理员后台侧边栏点击无反应问题（前后端联调 + 数据库校验）**：根因——proxy.ts 全局 IP 限流（100 req/min/IP，§2.6.4 第 6 项）未豁免已认证用户的后台浏览。一次侧边栏点击会产生 4~5 个伴随请求（HTML 页面 + get-session + unread-count + 业务 API + RSC prefetch），14 次点击 ≈ 60-70 请求，1 分钟内撑爆 100 限制，Next.js 客户端路由收到 429 JSON 后静默失败 → 用户表现为"侧边栏点击无反应"。静态代码分析无任何异常，必须实跑复现方定位。修复：proxy.ts 三层豁免策略——① HTML 页面（非 `/api/` 路径）一律豁免（限流目标仅防 API 滥用，不应阻碍已登录用户的后台浏览） ② 内部高频 API 前缀豁免（`/api/auth/` Better Auth 内部已限流 + `/api/notifications/` 客户端已节流 30s + `/api/health` 健康探针 + `/api/webhooks/` 三方回调） ③ 携带 Better Auth session cookie 的已认证请求豁免（cookie 由服务端 HMAC 签名，攻击者无法伪造有效签名；即使伪造 cookie 绕过限流，后续 API 仍会被 Better Auth 校验拒绝 401）。代码层：移除原 `RATE_LIMIT_SKIP_PATHS` Set 改为 `RATE_LIMIT_SKIP_PREFIXES` 数组 + `isRateLimitSkipped(pathname)` 函数；新增 `SESSION_COOKIE_NAME` 常量（默认 `better-auth.session_token`，可由 `BETTER_AUTH_SESSION_COOKIE` 覆盖）+ `isAuthenticated(request)` 函数（仅检查 cookie 存在性，签名验证由 Better Auth 路由层负责）；限流调用点条件改为 `!isRateLimitSkipped(pathname) && !isAuthenticated(request) && clientIp !== 'unknown'`。**移除**首次修复中曾加入的 `X-User-Id` 头检查——该头由前端 `src/lib/http.ts` 注入可被攻击者伪造，不可作为身份认证依据。验证：① 数据库一致性——init.sql 25 张表与 `prisma/schema.prisma` 完全一致；超管账号 `admin@example.com/admin123` 创建成功，role 字段正确写入 `super_admin` ② 前后端联调——`/api/auth/sign-in/email` 返回 200 + `role: "super_admin"`；`/api/auth/get-session` 正常；`/api/dashboard` 返回完整超管概览数据 ③ 浏览器自动化（agent-browser + production build + 1920×1080 viewport）——super_admin 全部 14 个侧边栏链接（用户管理 / 业务总览 / 收入明细 / 提现审核 / 工单客服 / 系统配置 / 审计日志 / 2FA 与 IP 白名单 / 更新面板 / 超管概览 / 概览 / 工单 / 通知 / 签到）跳转成功（"概览"跳到 /admin 是设计行为——/dashboard 自动按角色重定向）；SPEC.md §2.6.4 第 6 项同步补充豁免规则细节 |
| 1.6.4 | 2026-07-23 | **修复更新面板 `git: not found`——update-service 适配 Docker standalone 部署**：背景——超管点击"更新面板"侧边栏 → 页面调 `/api/admin/update/check` → `getCurrentVersion()` 执行 `execSync('git rev-parse HEAD')` → Docker runner 镜像无 git 二进制且无 .git 目录（standalone 产物不含）→ 抛 `获取本地版本失败:Command failed: git rev-parse HEAD /bin/sh: git: not found` → 接口 500 → 页面顶部红条报错，用户表现为"更新面板点不了"。整个 update 模块原设计假设是"完整 git 仓库部署"（git pull / npm install / prisma migrate / git reset），与 Docker standalone 部署模式根本冲突。修复（src/server/modules/update/update-service.ts 三处）：① `getCurrentVersion()` 三级降级不抛错——优先读 `DEPLOY_VERSION` 环境变量（Docker 部署时由 install.sh 注入宿主机 commit SHA）→ 回退 `git rev-parse HEAD`（开发模式/宿主机源码部署）→ 再回退占位值 `"unknown"`（Docker standalone 模式），保证面板可正常打开 ② 新增 `isGitAvailable()` 函数（try `git rev-parse --git-dir` 判断 git 可用性） ③ `executeUpdate()` 开头检测 Docker 模式，git 不可用直接抛明确错误指引"请在宿主机部署目录执行 `bash install.sh update` 完成更新"；`rollback()` 开头同理指引"请在宿主机执行 `bash install.sh reinstall` 重装"，避免后续 git pull/npm install/prisma migrate 一连串神秘失败。验证：production build + agent-browser 登录超管打开 /admin/update，页面正常渲染（侧边栏 14 项完整），`git: not found` 错误消失（check 接口降级返回 currentVersion=unknown）；tsc 自检 0 errors。遗留：更新面板完全可用还需在 .env 配置 `GITHUB_REPO_URL`/`GITHUB_REPO_BRANCH` 环境变量（install.sh 当前未自动生成，下版补） |
| 1.6.5 | 2026-07-23 | **修复移动端左上角汉堡菜单点击无反应——实现侧边栏抽屉**：背景——用户反馈"管理员/开发者后台左上角侧边栏按钮点击无反应"。根因——[topbar.tsx](src/components/layout/topbar.tsx) 第 47-49 行的汉堡菜单 `&#9776;`（☰）是个占位 `<span>`，**不是 button，没绑 onClick**，代码注释明写"M8.0 暂不实现抽屉，仅占位"。同时 [sidebar.tsx](src/components/layout/sidebar.tsx) 第 86 行 `hidden md:flex`——窄屏（<768px）下侧边栏完全隐藏，用户只能看到这个点了没反应的占位图标。桌面端（>=768px）侧边栏常驻显示，不受影响。修复（三文件联动）：① [layout.tsx](src/app/(dashboard)/layout.tsx) 加 `mobileNavOpen` state 提升到本层，供 Topbar（开）与 Sidebar（关）共享 ② [topbar.tsx](src/components/layout/topbar.tsx) 汉堡 `<span>` → `<button onClick={onMenuClick} aria-label="打开菜单">`，加 hover 态 ③ [sidebar.tsx](src/components/layout/sidebar.tsx) 提取 `SidebarContent` 组件复用桌面常驻与移动抽屉；移动端抽屉为 `fixed inset-0 z-50`（遮罩 `bg-black/40` 点击关闭 + 抽屉本体 `w-64` 左侧滑入），点击任意导航项 `onClick={onNavigate}` 自动关闭抽屉。三角色（super_admin/agent/developer）共享同一 Topbar+Sidebar，一并修复。验证：production build + agent-browser 375×812（iPhone）viewport 登录超管 → /admin 正常渲染 → 点击汉堡按钮抽屉弹出（全部 14 个导航链接可见）→ 点击"用户管理"跳转 /admin/users + 抽屉自动关闭；tsc 0 errors |
| 1.6.6 | 2026-07-23 | **修复 `MASTER_KEY 未配置`——install.sh / docker-compose.yml / .env.example 三处补齐**：背景——开发者创建应用时 `app-service.ts` 的 `getMasterKey()` 抛 `待接入：环境变量 MASTER_KEY 未配置（用于加密 RSA 私钥）`，导致 `/api/apps` POST 接口 500。MASTER_KEY 经 `deriveAesKey()`（SHA-256）派生为 AES-256 密钥，用于加密：① 开发者应用 RSA 私钥 + client_secret（app-service.ts） ② 卡密开发者水印（card-key-service.ts） ③ 易支付密钥（epay-service.ts）。根因——[install.sh](deploy/install.sh) `generate_env` 函数只生成了 DB_PASSWORD / REDIS_PASSWORD / BETTER_AUTH_SECRET / FIELD_ENCRYPTION_KEY 四个密钥，**漏了 MASTER_KEY**；[docker-compose.yml](docker-compose.yml) app 服务 environment 也未传递 MASTER_KEY；[.env.example](.env.example) 也未列出。修复（三文件）：① install.sh `generate_env` 加 `MASTER_KEY="$(openssl rand -hex 32)"`（遵循项目惯例，与 BETTER_AUTH_SECRET / FIELD_ENCRYPTION_KEY 同格式）+ 写入 .env（含注释说明用途与"生成后不可更改"警告） ② docker-compose.yml app 服务 environment 加 `MASTER_KEY: ${MASTER_KEY}` ③ .env.example 加 MASTER_KEY 段落说明。⚠️ **已部署环境升级注意**：`update` 命令不重新生成 .env（保留旧的），已部署用户拉新代码后需手动在 `/opt/jicek-wlyz/.env` 追加 `MASTER_KEY=$(openssl rand -hex 32 生成的值)`，然后 `docker compose restart app` 生效。安全性：因 MASTER_KEY 缺失前 createApp/card-key/epay 配置均会抛错，数据库中不存在已加密数据，所以首次补 MASTER_KEY 不会导致历史数据无法解密。验证：注入 MASTER_KEY 后重启 server，`POST /api/apps` 不再报 MASTER_KEY 未配置（getMasterKey 通过进入 prisma 阶段）；bash -n install.sh 语法 OK |
| 1.6.7 | 2026-07-23 | **实现彩虹易支付后台配置——config-service upsert + 加密 + 配置页新增 UI**：背景——用户问"彩虹易支付现在可以在后台配置吗"。排查发现完整链路缺失：① [epay-service.ts](src/server/modules/payment/epay-service.ts) `getEpayConfig()` 已实现从 SystemConfig 表读取 epay_pid/epay_key/epay_api_url，但 **init.sql 建表未预置这 3 行配置**，超管在 /admin/config 页面看到空表，无法配置 ② [config-service.ts](src/server/modules/config/config-service.ts) `updateSystemConfig` 用 `prisma.update`（非 upsert），配置行不存在时报错，超管无法首次创建 ③ `updateSystemConfig` 直接存明文 value，但 `epay_key` 应加密存储（`encrypted=true`），而 `epay-service.decryptEpayKey` 期望 `ivHex:cipherBase64` 格式——存读不对称 ④ 配置页只有编辑弹窗无"新增配置"入口。修复（两文件）：① config-service.ts 新增 `encryptConfigValue()`（MASTER_KEY SHA-256 派生 AES-256-CBC 加密，格式 `ivHex:cipherBase64`，与 app-service.encryptSecret 对称）+ `CONFIG_META` 元信息表（epay_pid 明文 / epay_key 加密 / epay_api_url 明文）+ `updateSystemConfig` 改用 `prisma.upsert`（已存在则更新，不存在按 CONFIG_META 创建）+ encrypted=true 的 value 写入前加密 ② [config/page.tsx](src/app/(dashboard)/admin/config/page.tsx) 加"新增配置"按钮 + Modal（输入 key/value，调用同一 PUT 接口 upsert）。验证：production build + agent-browser 登录超管 → /admin/config → 点"新增配置"填 epay_pid=10001 保存 → 表格出现 epay_pid 行（明文）→ 同理填 epay_key（密文存储 `ed5db1...:lelBiv...`，前端掩码 `******`，`encrypted:true`）→ epay_api_url → payment 分组 3 行齐全；tsc 0 errors。**用户配置步骤**：超管登录 → 系统配置 → 新增配置 → 依次填 3 个键（epay_pid 商户ID / epay_key 商户密钥自动加密 / epay_api_url 接口地址）→ 保存。依赖 v1.6.6 的 MASTER_KEY 已配置 |
| 1.6.8 | 2026-07-23 | **修复 `PLATFORM_RSA_PRIVATE_KEY 未配置` 与 `GITHUB_REPO_BRANCH 未配置`**：背景——用户反馈两个环境变量缺失报错。① `PLATFORM_RSA_PRIVATE_KEY`：[cloud-variable-service.ts](src/server/modules/cloud-variable/cloud-variable-service.ts) 和 [card-key-service.ts](src/server/modules/card-key/card-key-service.ts) 的 `getPlatformPrivateKey()` 读取此变量用于卡密 RSA 签名 / 云变量签名，但 install.sh 未生成 + docker-compose 未传递 + .env.example 未列出。技术难点：PEM 含换行，在 .env 文件和 docker-compose `environment` 传递时多行会被截断。修复：① [rsa.ts](src/lib/crypto/rsa.ts) 新增 `loadPrivateKeyFromEnv(envKey)` 公共函数——自动检测两种存储格式：以 `-----BEGIN` 开头则当 PEM 原文直接返回，否则 base64 解码后返回 PEM（兼容两种格式，向后兼容） ② cloud-variable-service.ts + card-key-service.ts 的 `getPlatformPrivateKey()` 改用 `loadPrivateKeyFromEnv('PLATFORM_RSA_PRIVATE_KEY')` ③ install.sh `generate_env` 加 `openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 | base64 -w 0` 生成 RSA-2048 私钥 base64 编码单行存储 ④ docker-compose.yml 传递 + .env.example 说明。② `GITHUB_REPO_URL` / `GITHUB_REPO_BRANCH`：[update-service.ts](src/server/modules/update/update-service.ts) 读取用于检查更新，[github webhook](src/app/api/webhooks/github/route.ts) 读取 BRANCH 校验推送分支。install.sh 默认填本项目仓库地址 `https://github.com/laobi465/jicek-wlyz.git` + `main` 分支，用户 fork 后可在 .env 手动修改。验证：node 内联脚本测试 base64 解码 → PEM → rsaSign → rsaVerify 闭环通过（签名长度 344，验签 ✓）；production server 注入环境变量后调 `/api/admin/update/check` 返回 `currentVersion=b2dc696` + `latestVersion` 完整数据，不再报 `GITHUB_REPO_BRANCH 未配置`；tsc 0 errors；bash -n install.sh 语法 OK。⚠️ **已部署环境升级注意**：`update` 不重新生成 .env，已部署用户需手动在 .env 追加：`PLATFORM_RSA_PRIVATE_KEY=$(openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 \| base64 -w 0)` + `GITHUB_REPO_URL=https://github.com/laobi465/jicek-wlyz.git` + `GITHUB_REPO_BRANCH=main`，然后 `docker compose restart app` |
| 1.6.9 | 2026-07-23 | **修复邮件/CDN/短信/备份/通用/对象存储无法后台配置——CONFIG_META 覆盖全部 7 分组 + 一键初始化默认配置**：背景——用户反馈"邮件，cdn，短信，备份，通用，对象存储，通用不能后台配置"。排查发现 v1.6.7 只为 payment 分组建了 CONFIG_META（3 键 epay_pid/epay_key/epay_api_url），其他 6 个分组（storage/email/sms/cdn/backup/general）的配置项在数据库 `system_configs` 表中**完全不存在空值行**，超管在 /admin/config 页面切换到这些分组时看到空表，既无法编辑也无法新增（updateSystemConfig 虽用 upsert 但无 CONFIG_META 元信息时 group 默认 general、encrypted 默认 false，导致 storage_secret_key / smtp_pass / sms_secret_key / cdn_secret_key 等敏感配置不会加密）。修复（两文件联动）：① [config-service.ts](src/server/modules/config/config-service.ts) `CONFIG_META` 从 3 项扩展到 **32 项覆盖全部 7 分组**——payment(3) / storage(7: storage_provider/access_key/secret_key 加密/bucket/region/endpoint/cdn_domain) / email(6: smtp_host/port/user/pass 加密/from/secure) / sms(5: sms_provider/access_key/secret_key 加密/sign_name/template_code) / cdn(3: cdn_domain/access_key/secret_key 加密) / backup(4: backup_schedule/retention_days/storage_type/path) / general(4: site_name/url/icp/contact_email)，敏感配置（SecretKey/密码）encrypted=true 自动加密存储 ② 新增 `initializeDefaultConfigs(operatorId)` 函数——遍历 CONFIG_META 为所有预定义项创建空值行（`findUnique` 已存在则跳过保证幂等），创建后写审计日志（AuditAction.CONFIG_UPDATE / targetId=batch_init） ③ [route.ts](src/app/api/admin/config/route.ts) 新增 `POST /api/admin/config` handler 调用 initializeDefaultConfigs 返回 `{created}`（鉴权 super_admin） ④ [config/page.tsx](src/app/(dashboard)/admin/config/page.tsx) import 加 `post` + `initLoading` state + `onInitDefaults()` 函数（window.confirm 二次确认 + 调 POST + toast 反馈） + PageHeader action 区加"初始化默认配置"按钮（与"新增配置"并列，loading 态防重复点击）。验证（production build + curl 全链路）：① 超管登录拿 session cookie ② POST /api/admin/config 返回 `created:29`（32 预定义 - 3 已存在 payment = 29 新建），数据库 `system_configs` 表 7 分组齐全（backup 4 / cdn 3 / email 6 / general 4 / payment 3 / sms 5 / storage 7 = 32 行） ③ 幂等性——再次 POST 返回 `created:0` "所有配置已存在" ④ 编辑明文配置 PUT /api/admin/config/smtp_host → value="smtp.qq.com" encrypted=false 正常 ⑤ 编辑加密配置 PUT /api/admin/config/smtp_pass → value="74a42ce...:FEbwcn..."（ivHex:cipherBase64 格式）encrypted=true 加密存储正常 ⑥ GET /api/admin/config?group=email 按分组查询正常返回 6 项；tsc 0 errors；next build 验证通过。**用户配置步骤**：超管登录 → /admin/config → 点"初始化默认配置"一键创建全部 32 项空值行 → 按分组筛选 → 点"编辑"逐项填入实际值（如 SMTP 密码会自动加密存储）。依赖 v1.6.6 MASTER_KEY 已配置 |
| 1.7.0 | 2026-07-23 | **优化 Docker 部署下"立即更新"提示——部署模式自适应 UI + 一键复制宿主机命令**：背景——用户反馈后台"立即更新"按钮在 Docker 容器化部署下点击后抛出 `当前为 Docker 容器化部署，容器内无法执行 git pull` 错误（v1.6.4 修复 getCurrentVersion 时加的 isGitAvailable 兜底），超管需翻看错误消息才知道要去宿主机执行 `bash install.sh update`，体验差且命令不可复制。经与用户确认方案（三选一：挂载 docker.sock 一键更新 / 优化提示+一键复制 / 异步更新队列），用户选最安全的"优化提示+一键复制"。修复（三文件联动）：① [update-service.ts](src/server/modules/update/update-service.ts) 新增 `getDeployMode()` 导出函数——基于 `isGitAvailable()`（检测 `git rev-parse --git-dir` 是否可执行）返回 `'source'`（源码部署，有 .git，支持后台 git pull）或 `'docker'`（Docker standalone，无 git/无 .git，需宿主机执行 install.sh） ② [check/route.ts](src/app/api/admin/update/check/route.ts) GET 响应 data 新增 `isDockerMode: boolean` 字段（`getDeployMode() === 'docker'`），前端据此切换 UI ③ [update/page.tsx](src/app/(dashboard)/admin/update/page.tsx) CheckResult 接口加 `isDockerMode` + 新增 `onCopyCommand()` 函数（navigator.clipboard.writeText 一键复制 + toast 反馈 + 不可用时提示手动选中）+ 版本信息卡片按钮区改为条件渲染：docker 模式展示"Docker 部署更新指引"卡片（warning 边框 + Badge 标识 + 两个命令块 `cd /opt/jicek-wlyz && bash install.sh update`（更新）/ `cd /opt/jicek-wlyz && bash install.sh reinstall`（回滚，保留 .env 与数据卷）+ 每个命令块独立"复制"按钮 + 三步操作说明：SSH 登录宿主机 → 粘贴执行 → 回本页点检查更新；路径非默认 /opt/jicek-wlyz 时提示替换）；source 模式保持原"触发更新"+"回滚上一版本"按钮。验证：① node 模拟无 .git 环境 → getDeployMode 返回 docker 模式（isDockerMode=true）→ 前端展示指引卡片 ② node 模拟有 .git 环境（源码部署）→ source 模式 → 展示触发更新按钮 ③ check API 首次调用 HTTP 200 返回完整版本数据（currentVersion=25481cc）证明路由正常 ④ tsc 0 errors；next build 通过。**安全权衡**：本方案不挂载 docker.sock（避免容器获取宿主机 root 权限），不引入宿主机 cron agent（避免额外运维组件），超管在后台看到清晰的命令指引 + 一键复制，SSH 到宿主机执行 install.sh update 完成更新，安全性最高 |
| 1.7.1 | 2026-07-23 | **修复配置页需手动初始化才能编辑——GET 自动补建缺失配置项**：背景——用户再次反馈"邮件，cdn，短信，备份，通用，对象存储，支付需要在后台可以直接修改配置"。v1.6.9 虽扩展了 CONFIG_META 到 32 项并加了"初始化默认配置"按钮，但超管需**先手动点初始化按钮**创建空值行后才能看到并编辑配置项，未初始化时切换到 storage/email/sms/cdn/backup/general 分组看到空表，体验差。修复（[config-service.ts](src/server/modules/config/config-service.ts)）：新增 `ensureDefaultConfigsExist()` 内部函数——先 `prisma.systemConfig.count()` 判断总数是否 < CONFIG_META 预定义数量（32），小于则遍历 CONFIG_META 逐项 `findUnique` 检查并 `create` 补建缺失的空值行（幂等，已存在不覆盖，不写审计日志因属自动行为）；`listSystemConfigs()` 在 `findMany` 查询前调用 `ensureDefaultConfigsExist()`，确保超管一打开 /admin/config 页面就自动看到全部 7 分组 32 项配置并直接编辑，无需手动初始化。验证（production build + curl 全链路）：① 删除非 payment 的 29 项配置（模拟只有 payment 3 项的初始环境）② GET /api/admin/config 返回 32 项（自动补建了 29 项），DB 各分组齐全（backup 4 / cdn 3 / email 6 / general 4 / payment 3 / sms 5 / storage 7）③ 编辑明文 cdn_domain → "https://cdn.test.com" 明文存储正常 ④ 编辑加密 smtp_pass → "6d6b071d...:HZtRho+RwJ..." ivHex:cipherBase64 加密存储正常 ⑤ GET ?group=sms 按分组查询返回 5 项；tsc 0 errors；next build 通过。**保留** v1.6.9 的"初始化默认配置"按钮作为手动触发入口（万一自动补建被跳过），但日常使用无需手动点——打开页面即自动补建 |
