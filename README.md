...
- **多语言接入**：6 种主流 SDK（Python/Java/PHP/Node.js/Go/易语言）+ 小众语言示例（gglua/andlua/auto.js/shell/按键精灵/html-js）
...
| M0 | 需求确认 + 文档基线 | 已完成 |
| M1 | 基础架构搭建 | 已完成 |
| M2 | 核心验证能力（加密链路 + 卡密 + 验证 API + 设备 + 云变量） | 进行中 |
| M3 | 商业化能力（代理分销 + 发卡 + 易支付 + 套餐充值） | 规划 |
| M4 | 接入生态（多语言 SDK + 接入中心向导） | 规划 |
| M5 | APK 注入（在线工具 + 命令行 + 反调试） | 规划 |
| M6 | 运营能力（工单 + 看板 + 签到 + 通知） | 规划 |
| M7 | 安全加固 + 正式上线 | 规划 |
