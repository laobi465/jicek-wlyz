...
- **Web 控制台**：三角色（开发者/代理/超管）RBAC 控制台，Better Auth 鉴权 + 工单/通知/签到闭环 + 开发者 8 模块管理页（应用/卡密/设备/云变量/APK注入/接入中心/店铺/套餐）+ 代理 5 模块管理页（代理概览/下级代理/邀请码/佣金明细/提现申请）（M8.0/M8.1/M8.2 已完成）
...
| M7 | 安全加固 + 正式上线 | 已完成 |
| M8 | Web 前端 UI（官网 + 三角色控制台 + 登录注册 + 仪表盘 + 工单/通知/签到 + 开发者 8 模块 + 代理 5 模块管理页） | 进行中（M8.0/M8.1/M8.2 已完成） |

> M8 拆分：M8.0 核心 UI 框架（已完成）→ M8.1 开发者管理页 + 官网（已完成）→ M8.2 代理管理页（已完成）→ M8.3 超管管理页。
