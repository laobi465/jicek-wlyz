...
- **Web 控制台**：三角色（开发者/代理/超管）RBAC 控制台，Better Auth 鉴权 + 工单/通知/签到闭环 + 开发者 8 模块管理页（应用/卡密/设备/云变量/APK注入/接入中心/店铺/套餐）+ 代理 5 模块管理页（代理概览/下级代理/邀请码/佣金明细/提现申请）+ 超管 10 模块管理页（超管仪表盘/用户/业务/收入/提现审核/工单客服/系统配置/审计日志/2FA+IP 白名单/更新面板）（M8.0/M8.1/M8.2/M8.3 已完成）
- **官网营销页**：产品介绍 + 核心特性 + 12 语言 SDK 展示 + 注册引导 CTA + 登录态感知导航（M8.1 已完成）
...
# 生产 Docker 部署时由 db 容器自动执行 deploy/init.sql 建表，无需手动运行
npm run db:push
...
| M7 | 安全加固 + 正式上线 | 已完成 |
| M8 | Web 前端 UI（官网 + 三角色控制台 + 登录注册 + 仪表盘 + 工单/通知/签到 + 开发者 8 模块 + 代理 5 模块 + 超管 10 模块管理页） | 进行中（M8.0/M8.1/M8.2/M8.3 已完成） |

> M8 拆分：M8.0 核心 UI 框架（已完成）→ M8.1 开发者管理页 + 官网（已完成）→ M8.2 代理管理页（已完成）→ M8.3 超管管理页（已完成）。
