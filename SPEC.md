...
> 版本：0.3.0 ｜ 状态：M1+M2 已完成，待进入 M3 商业化能力 ｜ 最后更新：2026-07-22
...
| 0.3.0 | 2026-07-22 | **M1+M2 完成**：M1 落地 Prisma 16 模型 Schema + Docker 多阶段构建 + docker-compose + 一键安装脚本（宝塔/Docker/端口冲突+1）+ Better Auth 三角色 RBAC + BullMQ 队列工厂 + /bdeploy GitHub 自动更新系统（HMAC-SHA256 Webhook + Redis 分布式锁 + 自动备份回滚）；M2 落地完整加密链路（RSA-2048 签名/加解密 + AES-256-CBC + ECDHE-PFS P-256 会话密钥 + TS 5min/Nonce 10min 防重放 + 滑窗限流 5/s 设备 + 100/min IP + 异地风控）+ 应用管理服务（AppKey/client_secret/RSA 密钥对生成）+ 卡密体系（7 类 + CRC32 校验位 + RSA 签名 + AES 开发者 ID 水印 + 同步≤100/BullMQ>100）+ 验证 API 统一入口 6 actions（verify_rsa/auth/use/unbind/check_update/heartbeat，resolveEncryptedContext 共享加密流程）+ 设备管理（绑定/解绑/心跳/上限校验）+ 云变量（公开/全部/设置/签名校验）；tsc --noEmit 自检 0 errors |
