...
> 版本：0.1.1 ｜ 状态：规划中（未进入编码）｜ 最后更新：2026-07-22
...
| ICP 备案（国内服务器） | 待核实 | 待用户确认是否国内服务器 |
| 服务器配置未知 | Docker 资源限制无法定稿 | 待用户补充 CPU/内存/带宽 |
| 通知服务（邮件/短信）未选型 | 工单/验证码无法发送 | 待用户确认 |
...
| # | 项目 | 状态 |
|---|---|---|
| 1 | 服务器配置（CPU/内存/带宽） | 待核实 |
| 2 | 是否前置 Cloudflare CDN | 待核实 |
| 3 | 是否需要 ICP 备案 | 待核实 |
| 4 | 邮件服务 SMTP 选型 | 待核实 |
| 5 | 短信服务选型 | 待核实 |
| 6 | 套餐定价 + 应用数/卡密额度具体数值 | 待核实 |
| 7 | 代理佣金默认比例（3 层各 %） | 待核实 |
| 8 | 代理提现门槛 + 结算周期 | 待核实 |
| 9 | 自定义小时卡范围 | 待核实 |
| 10 | 数据库备份频率 + 保留天数 | 待核实 |
| 11 | 日志保留天数 | 待核实 |
| 12 | 是否需要监控告警（Prometheus + Grafana） | 待核实 |
| 13 | 是否需要灰度/蓝绿部署 | 待核实 |
| 14 | 超管初始账号生成方式 | 待核实 |
| 15 | 后台是否需要 i18n | 待核实 |
| 16 | 是否需要数据导出（Excel） | 待核实 |
| 17 | 用户协议/隐私政策模板 | 待核实 |
| 18 | SDK 旧版本兼容策略 | 待核实 |
| 19 | SSH 脚本托管 URL（推荐 jsDelivr） | 待核实 |
