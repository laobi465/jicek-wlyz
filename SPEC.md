...
| 1.6.9 | 修复邮件/CDN/短信/备份/通用/对象存储无法后台配置：v1.6.7 只为 payment 分组建了 CONFIG_META（3 键），其他 6 分组（storage/email/sms/cdn/backup/general）的配置项在 `system_configs` 表完全不存在空值行，超管切换分组看到空表无法配置。修复：① config-service.ts `CONFIG_META` 从 3 项扩展到 32 项覆盖全部 7 分组（storage 7/email 6/sms 5/cdn 3/backup 4/general 4 + payment 3），敏感配置（SecretKey/密码）encrypted=true 自动加密 ② 新增 `initializeDefaultConfigs(operatorId)` 遍历 CONFIG_META 创建空值行（findUnique 已存在跳过保证幂等 + 写审计日志） ③ route.ts 新增 `POST /api/admin/config` 调用 initializeDefaultConfigs 返回 {created} ④ config/page.tsx 加"初始化默认配置"按钮（window.confirm 确认 + loading 态）+ onInitDefaults() 调 POST。验证：POST 返回 created:29（32-3 已存在），7 分组 32 行齐全；幂等 created:0；明文/加密配置编辑正常（smtp_pass 加密为 ivHex:cipherBase64）；tsc 0 errors；next build 通过 | 已完成 |
| 1.7.0 | 优化 Docker 部署下"立即更新"提示——部署模式自适应 UI + 一键复制宿主机命令：v1.6.4 在 executeUpdate 开头加 isGitAvailable 兜底，Docker 模式抛"容器内无法 git pull"错误，超管需翻错误消息找命令且不可复制。修复：① update-service.ts 新增 `getDeployMode()` 导出（isGitAvailable ? 'source' : 'docker'） ② check/route.ts 响应 data 加 `isDockerMode: boolean` ③ update/page.tsx CheckResult 加 isDockerMode + onCopyCommand()（navigator.clipboard 一键复制+toast）+ 版本卡片按钮区条件渲染：docker 模式展示"Docker 部署更新指引"卡片（两个命令块 `cd /opt/jicek-wlyz && bash install.sh update`/`reinstall` + 各自"复制"按钮 + 三步说明），source 模式保持触发更新/回滚按钮。验证：无 .git 环境→docker 模式→指引卡片；有 .git→source→按钮；check API 200 正常；tsc 0 errors；build 通过。不挂载 docker.sock（避免宿主 root 权限），不引入 cron agent，安全性最高 | 已完成 |
| 1.7.1 | 修复配置页需手动初始化才能编辑——GET 自动补建缺失配置项：v1.6.9 虽扩展 CONFIG_META 到 32 项并加了"初始化默认配置"按钮，但超管需先手动点初始化才能看到并编辑配置项，未初始化时切换分组看到空表。修复：config-service.ts 新增 `ensureDefaultConfigsExist()` 内部函数——先 count 判断总数是否 < CONFIG_META 预定义数量（32），小于则遍历逐项 findUnique 检查并 create 补建缺失空值行（幂等，不写审计日志）；`listSystemConfigs()` 在 findMany 前调用 ensureDefaultConfigsExist()，超管一打开页面就自动看到全部 7 分组 32 项配置并直接编辑。验证：删非 payment 29 项→GET 返回 32 项自动补建 29 项；编辑明文 cdn_domain/加密 smtp_pass 正常；tsc 0 errors；build 通过 | 已完成 |
...
| 1.7.0 | 2026-07-23 | **SDK 下载与对接教程页面**：背景——用户要求开发者和管理员后台均能下载各语言 SDK 并查看对接教程。新增 GET /api/sdk/info（无需鉴权，从 GITHUB_REPO_URL/GITHUB_REPO_BRANCH 环境变量拼接 GitHub raw 下载基础 URL，返回 6 主流 SDK + 6 社区示例 + 6 步接入流程 + 6 核心 API 协议 + 仓库信息；复用 access-service.listSdkLanguages/ACCESS_STEPS，不重复维护语言清单）/ 共享组件 src/components/sdk/sdk-download-content.tsx（主流 SDK 卡片网格 下载/复制链接 + 社区示例卡片 + 6 步对接流程列表 + API 协议表格 verify_rsa/auth/heartbeat/check_update/use/unbind + 加密方式 Badge + Python 接入完整示例 verify_rsa→auth→heartbeat→check_update→use→unbind 一键复制）/ 新增 /developer/sdk + /admin/sdk 两个独立页面（分别 AuthGuard allow developer/super_admin，复用 SdkDownloadContent 内容组件）/ sidebar DEVELOPER_NAV 在"接入中心"后新增"SDK 下载"入口、ADMIN_NAV 在"更新面板"前新增"SDK 下载"入口 + 顶部注释同步；tsc 自检 0 errors；next build 验证通过 |
